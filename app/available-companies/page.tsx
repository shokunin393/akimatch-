...
export default function AvailableCompanies() {
  ...
  const filteredCompanies = availableCompanies
    .filter((company) => {
      ...
      const matchesPrefecture = !selectedPrefecture || company.prefectures.includes(selectedPrefecture)

      const matchesWorkType =
        selectedWorkTypes.length === 0 || selectedWorkTypes.some((type) => company.workTypes.includes(type))

      // 期間フィルター
      const matchesDateRange =
        (!startDate || company.availableTo >= startDate) && (!endDate || company.availableFrom <= endDate)
            ...
            <div className="bg-white rounded-lg shadow-md p-4 mb-6 border border-blue-100">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                  ...
                  <p className="text-blue-600">{filteredCompanies.length}社・事業者が見つかりました</p>
                </div>
            ...
            {viewMode === "card" ? (
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                {filteredCompanies.map((company) => (
                  <Card key={company.id} className="hover:shadow-lg transition-shadow border border-blue-100">
                    <CardHeader className="pb-2">
                      <div className="flex justify-between items-start mb-2">
                        <div className="flex flex-wrap gap-1">
                          {company.workTypes.slice(0, 2).map((workType) => (
                            <Badge key={workType} variant="secondary" className="text-xs">
                              {workType}
                            </Badge>
                          ))}
                          {company.workTypes.length > 2 && (
                      ...
                      <CardTitle className="text-lg text-blue-800">{company.companyName}</CardTitle>
                    ...
                    <CardContent className="pb-2">
                      <div className="space-y-2 text-sm text-gray-600">
                        <div className="flex items-center">
                          ...
                          {company.prefectures.join("、")}
                        </div>
                        <div className="flex items-center">
                          <CalendarDays className="h-4 w-4 mr-2 text-blue-500" />
                          対応可能期間: {company.period}
                        </div>
                        <div className="flex items-center">
                      ...
                      {filteredCompanies.map((company) => (
                        <tr key={company.id} className="hover:bg-blue-50">
                          <td className="px-4 py-3 whitespace-nowrap">
                            <div className="text-sm font-medium text-blue-800">{company.companyName}</div>
                            <div className="text-xs text-gray-500">{company.description.substring(0, 30)}...</div>
                          </td>
                          ...
                          <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-700">{company.period}</td>
                          <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-700">{company.contactPerson}</td>
